...
object Quinoa_2_Docker_Image : Template({
    ...
    steps {
        ...
        script {
            ...
            id = "RUNNER_24"
            workingDir = "%workdir%"
            scriptContent = "docker build --build-arg http_proxy=http://proxyout.lanl.gov:8080/ --build-arg https_proxy=http://proxyout.lanl.gov:8080/ --build-arg COMMIT=%build.vcs.number% --no-cache=true --rm=true -t %organization%/%repository%-build:%tag% -f %dockerfile% ."
        }
        script {
            name = "Squash image"
            id = "RUNNER_30"
