...
object Quinoa_2_Mac_Matrix : Template({
    ...
    val stepPrefix = """
      ...
      [ %compiler% == gnu ] && port select gcc mp-gcc7 && port select mpi openmpi-gcc7-fortran
    """.trimIndent()
    ...
    steps {
        ...
        script {
            ...
            scriptContent = """
                ...
                cmake -DCMAKE_CXX_COMPILER=mpicxx -DCMAKE_C_COMPILER=mpicc -DCMAKE_BUILD_TYPE=%buildtype% -DCMAKE_CXX_FLAGS=-Werror -DTPL_DIR=/Users/jbakosi/code/quinoa-tpl/install/%compiler%-x86_64%tpl% -DCMAKE_CXX_COMPILER_LAUNCHER=ccache ../src && ccache -z && make -j%teamcity.agent.hardware.cpuCount% && ccache -s
            """.trimIndent()
        }
        script {
            name = "Run tests"
