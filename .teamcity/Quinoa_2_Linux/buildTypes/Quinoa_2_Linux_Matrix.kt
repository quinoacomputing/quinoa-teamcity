...
object Quinoa_2_Linux_Matrix : Template({
    ...
    val stepPrefix = """
      ...
      [ %compiler% == clang ] && module load clang/latest openmpi/3.1.1/clang/latest hdf5/clang netcdf/clang hypre/clang
      ...
      [ %compiler% == intel ] && module load intel/2018 openmpi/3.1.1/intel/2018 hdf5/intel netcdf/intel hypre/intel root/intel
    ...
    steps {
        ...
        script {
            ...
            scriptContent = """
                ...
                cmake -DCMAKE_CXX_COMPILER=mpicxx -DCMAKE_C_COMPILER=mpicc -DCMAKE_BUILD_TYPE=%buildtype% -DSTDLIBCPP=%stdlibcpp% -DCMAKE_DISABLE_FIND_PACKAGE_RNGSSE2=!%rngsse2% -DCMAKE_DISABLE_FIND_PACKAGE_TestU01=!%testu01% -DCMAKE_CXX_FLAGS=-Werror -DRUNNER_ARGS="--bind-to none -oversubscribe" -GNinja ../src && ninja -j8
            """.trimIndent()
        }
        script {
            name = "Run tests"
            ...
            scriptContent = """
                ${stepPrefix}
                if [ %smp% ]; then ./charmrun +p6 "--bind-to none -oversubscribe" Main/unittest -v -q +ppn 3; else ./charmrun +p8 "--bind-to none -oversubscribe" Main/unittest -v -q; fi && ctest -j8 --output-on-failure -LE extreme
            """.trimIndent()
        }
    }
